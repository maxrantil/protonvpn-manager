...
test_no_hardcoded_paths() {
	...
	for file in "${files_to_check[@]}"; do
		if [[ -f "$file" ]]; then
			...
			if grep -q "/home/mqx/workspace/claude-code/vpn" "$file" 2>/dev/null; then
				# Allow detection but not usage
				if grep -v "# Security check\|WARNING\|ERROR" "$file" | grep -q "/home/mqx/workspace/claude-code/vpn"; then
